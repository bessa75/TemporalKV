...
"""
This file features our custom Cache implementation with the classes SparseAccumulator and DynamicCache
Our original code is from lines 446 to 767
"""
...
class SparseAccumulator:
    def __init__(self, device, dtype, bsz, q_leni, num_heads, head_dim, n_layers, t):
        self.device = device
        self.dtype = dtype
        self.bsz = bsz
        self.q_leni = q_leni
        ...
        self.max_nnz = int(bsz * q_leni * num_heads * head_dim * 1.5 * 2 * t )
        self.indices = torch.empty((n_layers, 4, self.max_nnz), dtype=torch.int16, device=device)
        self.values = torch.empty((n_layers, self.max_nnz,), dtype=self.dtype, device=device)
        self.token_offset = [0 for i in range (n_layers)]
        self.n_outliers = [0 for i in range (n_layers)]

    @line_profiler.profile
    def append(self, tensor, mask, layer_idx):  # expected to be a coalesced sparse tensor
        sparse_block = self.convert_to_outliers(tensor,mask)
        # print(f"n outliers pre {self.n_outliers[layer_idx]}")
        idx = sparse_block.indices().to(torch.int16)  # (ndim, nnz)
        val = sparse_block.values()   # (nnz,)

        idx = idx.clone()
        assert idx.shape[0] == 4
        # print(f"token_offset : {self.token_offset[layer_idx]}")
        idx[2] += self.token_offset[layer_idx]
        if self.token_offset[layer_idx] + idx.shape[1] > self.max_nnz:
            raise Exception("Max sparse cache length reached")

        self.indices[layer_idx, :, self.n_outliers[layer_idx]:self.n_outliers[layer_idx]+idx.shape[1]] = idx.to(self.device)
        self.values[layer_idx, self.n_outliers[layer_idx]:self.n_outliers[layer_idx]+idx.shape[1]] = val.to(self.device)

        self.token_offset[layer_idx] += sparse_block.shape[2]
        self.n_outliers[layer_idx] += idx.shape[1]
        # print(f"n outliers post {self.n_outliers[layer_idx]}")

        return self._current_tensor(layer_idx)

    @line_profiler.profile
    def _current_tensor(self, layer_idx):
        all_indices = self.indices[layer_idx, :, :self.n_outliers[layer_idx]].to(torch.long)
        all_values  = self.values[layer_idx, :self.n_outliers[layer_idx]]
        shape = (self.bsz,self.num_heads,self.token_offset[layer_idx],self.head_dim)

        assert all_indices.dtype == torch.long
        assert all_indices.device == all_values.device

        return all_indices.cuda(),all_values.cuda()
    
    @line_profiler.profile
    def convert_to_outliers(self, tensor, mask):
        outlier_indices = mask.nonzero(as_tuple=False)
        outlier_indices = outlier_indices.T
        outlier_values = tensor[tuple([outlier_indices[i] for i in range (4)])]
        sparse_outlier_tensor = torch.sparse_coo_tensor(
...
class DynamicCache(Cache):
    ...
    def __init__(self,
        ...
        self.nb_sink_tokens = nb_sink_tokens
        self.nb_local_tokens = nb_local_tokens
        self.n_layers = None
    ...
    def init_n_layers(self, bsz, q_len, num_heads, head_dim, outlier,t = None,quant_type = "unspecified",size_centroids = -1,offset = 0):
        #tot_head_dim*n_tokens*0.01*margin
        self.t = None
        self.seen_tokens=0
        self.is_collect = False
        self.key_cache = [None for i in range (0,self.n_layers)]
        self.value_cache = [None for i in range (0,self.n_layers)]
        self.sink_tokens_k = [None for i in range (0,self.n_layers)]
        self.sink_tokens_v = [None for i in range (0,self.n_layers)]
        self.local_tokens_k = [None for i in range (0,self.n_layers)]
        self.local_tokens_v = [None for i in range (0,self.n_layers)]
        self.key_lower_quantiles = [None for i in range (0,self.n_layers)]
        self.key_upper_quantiles = [None for i in range (0,self.n_layers)]
        self.outlier = outlier
        assert quant_type != "unspecified"
        self.quant_type = quant_type
        self.size_centroids = size_centroids
        self.offset = offset
        self.sum_key_proportion = 0
        self.nb_sum_key = 0
        if self.outlier:
            self.t = t 
            self.value_sparse = SparseAccumulator("cuda:1",torch.bfloat16, bsz, q_len, num_heads, head_dim, self.n_layers, self.t)
            self.key_sparse = SparseAccumulator("cuda:1",torch.bfloat16, bsz, q_len, num_heads, head_dim, self.n_layers, self.t)
    ...
    def update(
        ...
        Parameters:
            key_states (`torch.Tensor`): (bsz, self.num_heads, q_len, self.head_dim)
                The new key states to cache.
            value_states (`torch.Tensor`):
                The new value states to cache.
            layer_idx (`int`):
        ...
        Return:
            A tuple containing the updated key and value states.
        """
        if "key_codes" in cache_kwargs:
            key_codes = cache_kwargs["key_codes"]
            value_codes = cache_kwargs["value_codes"]
            n_centroids = cache_kwargs["n_centroids"]

            #Detect outliers 
            if self.outlier:
                #Keys
                assert self.key_upper_quantiles is not None
                assert self.key_lower_quantiles is not None
                # is_below_quantiles_key = key_states <= self.key_lower_quantiles[layer_idx]
                # is_upper_quantiles_key = key_states >= self.key_upper_quantiles[layer_idx]
                # is_outlier_key = torch.logical_or(is_below_quantiles_key,is_upper_quantiles_key)
                # outlier_key = is_outlier_key * key_states
                upper_vals, upper_indices = torch.topk(key_states,int(self.t * key_states.shape[-2]),dim = -2)
                lower_vals, lower_indices = torch.topk(key_states,int(self.t * key_states.shape[-2]),dim = -2,largest=False)
                tot_vals = torch.cat([lower_vals,upper_vals],dim = -2)
                tot_indices = torch.cat([lower_indices,upper_indices],dim = -2)
                vals2 = torch.zeros_like(key_states)
                vals2.scatter_(-2, tot_indices, tot_vals)
                # print(f"proportion of key outliers : {torch.mean((is_outlier_key != 0).float())}")
                # self.sum_key_proportion +=  torch.sum((is_outlier_key != 0).float())
                # self.nb_sum_key += torch.sum(torch.ones(is_outlier_key.shape))

                # print(f"mean proportion of key outliers : {self.sum_key_proportion / self.nb_sum_key}")

                # print(f"number of key outliers : {torch.sum(is_outlier_key.float())}")
                # print(f"number of keys : {key_states.flatten().shape[0]}")
                key_sparse = self.key_sparse.append(key_states,vals2!=0,layer_idx)
                # key_sparse = self.key_sparse.append(key_states,is_outlier_key,layer_idx)
                #Values
                assert self.t is not None
                value_states_reshaped = value_states.transpose(1,2).view(value_states.shape[0]*value_states.shape[2],value_states.shape[1]*value_states.shape[3])
                # print(value_states_reshaped.dtype)
                value_lower_quantiles = torch.quantile(value_states_reshaped.float(),self.t,dim = 1).reshape(value_states.shape[0],1,value_states.shape[2],1)
                value_upper_quantiles = torch.quantile(value_states_reshaped.float(),1-self.t,dim = 1).reshape(value_states.shape[0],1,value_states.shape[2],1)
                is_below_quantiles_value = value_states <= value_lower_quantiles
                is_upper_quantiles_value = value_states >= value_upper_quantiles
                is_outlier_value = torch.logical_or(is_below_quantiles_value, is_upper_quantiles_value)
                
                value_sparse = self.value_sparse.append(value_states,is_outlier_value,layer_idx)
                #print(f"proportion of value outliers : {torch.mean(is_outlier_value.float())}")

                # print(key_codes.shape, value_codes.shape)
                # print(is_outlier_key.shape, is_outlier_value.shape)
                # key_codes = key_codes * (~is_outlier_key).int().unsqueeze(-1) + (n_centroids-1) * is_outlier_key.int().unsqueeze(-1)
                # print(f"n_centroids {n_centroids}")
                # value_codes = value_codes * (~is_outlier_value).int().unsqueeze(-1) + (n_centroids-1) * is_outlier_value.int().unsqueeze(-1)
            # print("here0")

                 
            # Update the cache
            if key_states.shape[2]>1: #we dont compress generated tokens
                # if layer_idx==0:
                    # print("codes appending")
                if self.key_cache[layer_idx] is None:
                    if self.first_layer is None:
                        self.first_layer=layer_idx
                    self.key_cache[layer_idx]=key_codes
            ...
            #     if layer_idx==0:
            #         self.seen_tokens+=1
                    # print("no codes appending")
                    # print(key_states.shape)

            assert len(self.sink_tokens_k)==len(self.sink_tokens_v)
            assert len(self.local_tokens_k)==len(self.local_tokens_v)

            #Update sink tokens
            if self.sink_tokens_k[layer_idx] is None:
                self.sink_tokens_k[layer_idx] = key_states[:,:,:min(self.nb_sink_tokens,key_states.shape[-2])]
                self.sink_tokens_v[layer_idx] = value_states[:,:,:min(self.nb_sink_tokens,value_states.shape[-2])]
                #bsz, self.num_heads, q_len, self.head_dim
            else:
                nb_sink_registered = self.sink_tokens_k[layer_idx].shape[-2]
                assert nb_sink_registered == self.sink_tokens_v[layer_idx].shape[-2]
                if nb_sink_registered < self.nb_sink_tokens :
                    diff=self.nb_sink_tokens-nb_sink_registered
                    self.sink_tokens_k[layer_idx] = torch.cat([self.sink_tokens_k[layer_idx],key_states[:,:,:diff]],dim=-2)
                    self.sink_tokens_v[layer_idx] = torch.cat([self.sink_tokens_v[layer_idx],value_states[:,:,:diff]],dim=-2)

            #Update local tokens
            if self.local_tokens_k[layer_idx] is None:
                assert key_states.shape[-2]==value_states.shape[-2]
                if key_states.shape[-2]<=self.nb_local_tokens:
                    self.local_tokens_k[layer_idx] = key_states
                    self.local_tokens_v[layer_idx] = value_states
                elif self.nb_local_tokens!=0:
                    self.local_tokens_k[layer_idx] = key_states[:,:,key_states.shape[-2]-self.nb_local_tokens:]
                    self.local_tokens_v[layer_idx] = value_states[:,:,value_states.shape[-2]-self.nb_local_tokens:]
            else:
                combined_k = torch.cat([self.local_tokens_k[layer_idx],key_states],dim=-2)
                combined_v = torch.cat([self.local_tokens_v[layer_idx],value_states],dim=-2)
                assert combined_k.shape[-2]==combined_v.shape[-2]
                if combined_k.shape[-2]<=self.nb_local_tokens:
                    self.local_tokens_k[layer_idx]=combined_k
                    self.local_tokens_v[layer_idx]=combined_v
                elif self.nb_local_tokens!=0:
                    self.local_tokens_k[layer_idx]=combined_k[:,:,combined_k.shape[-2]-self.nb_local_tokens:]
                    self.local_tokens_v[layer_idx]=combined_v[:,:,combined_v.shape[-2]-self.nb_local_tokens:]
                else:
                    assert self.local_tokens_k[layer_idx] is None and self.local_tokens_v[layer_idx] is None
                # if layer_idx == self.first_layer:
                #     self.seen_tokens += key_states.shape[-2]
            if self.outlier:
                return self.key_cache[layer_idx], self.value_cache[layer_idx], key_sparse, value_sparse, self.sink_tokens_k[layer_idx], self.sink_tokens_v[layer_idx], self.local_tokens_k[layer_idx], self.local_tokens_v[layer_idx]
            else:
                return self.key_cache[layer_idx], self.value_cache[layer_idx], self.sink_tokens_k[layer_idx], self.sink_tokens_v[layer_idx], self.local_tokens_k[layer_idx], self.local_tokens_v[layer_idx]
        else:
            # if layer_idx == 0:
            # self.seen_tokens += key_states.shape[-2]
            # Update the cache
            if self.is_collect and len(self.key_cache)==0:
                self.key_cache = [None for i in range (0,32)]
                self.value_cache = [None for i in range (0,32)]
                print("IS_COLLECT IS TRUE")
    ...
    def get_seq_length(self, layer_idx: Optional[int] = 0) -> int:
        ...
        if len(self.key_cache)<=layer_idx or self.key_cache[layer_idx] is None:
            return 0
        if self.quant_type == "chan" or self.quant_type == "noquant":
            if self.local_tokens_k[layer_idx] is not None :
                return self.key_cache[layer_idx].shape[-2]+self.local_tokens_k[layer_idx].shape[-2]
            else:
                return self.key_cache[layer_idx].shape[-2]
        else:
            if self.quant_type != "norm":
                if self.local_tokens_k[layer_idx] is not None :
                    return self.key_cache[layer_idx].shape[-3]*self.size_centroids+self.local_tokens_k[layer_idx].shape[-2]
                else:
                    return self.key_cache[layer_idx].shape[-3]*self.size_centroids
            else:
                if self.local_tokens_k[layer_idx] is not None :
                    return self.key_cache[layer_idx].shape[-3]*self.size_centroids+self.local_tokens_k[layer_idx].shape[-2]+self.offset
                else:
                    return self.key_cache[layer_idx].shape[-3]*self.size_centroids+self.offset
