...
class MultiKMeans:
  ...
  def euc_sim(a, b):
    """
      ...
      a: torch.Tensor, shape: [m, n_features]
      b: torch.Tensor, shape: [n, n_features]
      fisher_dir: str, type of fisher clustering
  ...
  def fit_predict(self, X, centroids=None,weights=None,fisher_dir=None):
    """
    ...
    for i in range(self.max_iter):
      ...
      mask = (expanded_closest==arranged_mask).to(X.dtype) #(bsz,n_clusters,n_points)
      #weights has shape (bsz,n_points,size_centroids)
      if weights is not None: 
        assert fisher_dir in ["fisher","fisher_kvquant","uniform"]
        if fisher_dir in ["fisher","fisher_kvquant"]:
          mask=mask*(weights.sum(dim=-1).unsqueeze(1))
