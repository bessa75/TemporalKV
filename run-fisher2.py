...
###################################################################
###################################################################
###### This code is a modified version of a file from       #######
###### KVQuant's GitHub repository                          #######
###### We do not claim any form of authorship over it       #######
###### See : https://github.com/SqueezeAILab/KVQuant        #######
###################################################################
###################################################################
...
@dataclass
class DataArguments:
    dataset: str = field(default="nodataset")
    num_examples: int = field(
        default=16, metadata={"help": "Number of calibration examples"}
    )
...
def train():
    ...
    # main loop
    for k, data in tqdm(enumerate(dataloader[: data_args.num_examples])):
        data = data[0]
        x = data.cuda()
        model.zero_grad()
        # act gradients
        ...
        for i, layer in enumerate(_layers):
            ...
            kact = k_proj.act.float().cpu()
            vact = v_proj.act.float().cpu()

            k_proj.act = None
            v_proj.act = None

            if f"k_proj{i}" not in grads or grads[f"k_proj{i}"] is None:
                grads[f"k_proj{i}"] = kgrad
            else:
                grads[f"k_proj{i}"] = torch.cat((grads[f"k_proj{i}"], kgrad), dim=0)
            if f"v_proj{i}" not in grads or grads[f"v_proj{i}"] is None:
                grads[f"v_proj{i}"] = vgrad
            else:
                grads[f"v_proj{i}"] = torch.cat((grads[f"v_proj{i}"], vgrad), dim=0)
            if f"k_proj{i}" not in acts or acts[f"k_proj{i}"] is None:
                acts[f"k_proj{i}"] = kact
            else:
                acts[f"k_proj{i}"] = torch.cat((acts[f"k_proj{i}"], kact), dim=0)
            if f"v_proj{i}" not in acts or acts[f"v_proj{i}"] is None:
                acts[f"v_proj{i}"] = vact
            else:
                acts[f"v_proj{i}"] = torch.cat((acts[f"v_proj{i}"], vact), dim=0)
        print("collection done")
        if k % 8 == 7:
            print("___________________")
            print(f"saving split {k//8} at {training_args.output_dir}")
            for i, layer in enumerate(_layers):
                tensor_dict = {
                    "grads_v": grads[f"v_proj{i}"],
                    "acts_v": acts[f"v_proj{i}"],
                    "grads_k": grads[f"k_proj{i}"],
                    "acts_k": acts[f"k_proj{i}"],
                }
                ...
                output_file = (
                    ...
                    + model_name.split("/")[-1]
                    + "/"
                    + f"layer{i}_nb{data_args.num_examples}"
                    + "split"
                    + str(k // 8)
                    + ".safetensors"
                )
